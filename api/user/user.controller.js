const {
  ...
  addAdminByAdmin,
  updatelastlogin,
  updateandRegisterUser,
  forgotPassword,
  getRegUserByUserEmail,
...
module.exports = {
  signUp: (req, res) => {
    ...
    getUserByUserEmail(body.email, (err, result) => {
      ...
      }else if(result && result.type=='guest')
      {
        const salt = genSaltSync(10);
        body.password = hashSync(body.password, salt);
        body.type = "user";
        updateandRegisterUser(body,result.user_id, (err, results) => {
          if (err) {
            console.log(err);
            return res.status(500).json({
              success: 0,
          ...
          sendMail(msg,cred, token, (err, result) => {
            if (err) {
              return res.json({
                status: 0,
                error: err,
  ...
  forgotPassword: (req, res) => {
    ...
    if (email) {
      getRegUserByUserEmail(email, (err, results) => {
        if (err) {
          console.log(err);
          return res.status(500).json({
            success: 0,
        ...
        if (results) {
          ...
          forgotPassword(msg,email, sendtoken, (err, body) => {
            if (err) {
              return res.json({
                status: 0,
                error: err,
                message: "Cannot Send Confirmation mail",
              });
            }
            return res.status(200).json({
              success: 1,
        ...
        return res.status(403).json({
          success: 2,
          message: "!This Email isn't associated with any account",
          error: err,
        });
      }
      });
