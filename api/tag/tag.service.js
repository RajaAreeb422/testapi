...
module.exports = {
  addTag: (data) => {
    ...
    {
      pool.query(
        `insert into maz_ecommerce.tag set name=?`,
          [
          data.name,
  ...
  getTagById: (id, callBack) => {
    pool.query(
      `select * from maz_ecommerce.tag where id = ?`,
      [id],
      (error, results, fields) => {
        if (error) {
          return callBack(error,null);
  ...
  deleteTagById: (id, callBack) => {
    pool.query(
      `delete from maz_ecommerce.tag where id = ?`,
      [id],
      (error, results, fields) => {
        if (error) {
