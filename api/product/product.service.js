...
addProductVariantDetails = (id, valueId) => {
  ...
  return new Promise((resolve, reject) => {
    let sql =
      "INSERT INTO maz_ecommerce.product_variant_details set product_variant_id= ?,variant_value_id=? ";
...
module.exports = {
  addProduct: (data) => {
    return new Promise((resolve, reject) => {
      if (data.variants) {
        ...
        pool.query(
          `insert into maz_ecommerce.product set name=?,category_id=?,collection_id=?,supplier_id=?,  product_description=?,regular_price=?, has_variant=1`,
          [
            data.name,
            data.category_id,
            data.collection_id,
       ...
       // let iid=Math.floor(Math.random() * 10)
        pool.query(
          `insert into maz_ecommerce.product set name=?,category_id=?,collection_id=?,supplier_id=?,product_description=?,sku=?,regular_price=?,stock_quantity=?,low_stock_threshold=?,discount_price=?`,
          [
  ...
  getProductBySKU: (sku, variant) => {
    return new Promise((resolve, reject) => {
      ...
      pool.query(
        `select id from maz_ecommerce.${table} where sku=?`,
        [sku],
        (error, result, fields) => {
  ...
  addProductVariants: (id, combinations) => {
    ...
    let new_sku=combinations.sku +id
    return new Promise((resolve, reject) => {
      let sql =
        "INSERT IGNORE INTO maz_ecommerce.product_variants (product_id,product_variant_name,sku,regular_price,stock_quantity,low_stock_threshold) values ";
  ...
  getAllProducts: (callBack) => {
    pool.query(
      `select p.id,p.name,p.collection_id,p.supplier_id,p.category_id, p.regular_price,pi.path from maz_ecommerce.product as p left join maz_ecommerce.product_images as pi on p.id=pi.product_id `,
      [],
      (error, results, fields) => {
        if (error) {
          return callBack(error, null);
  ...
  getProductandVariantsById: (id, callBack) => {
    // group by v.name
    pool.query(
       `SELECT p.name as product_name,p.supplier_id,p.category_id,p.regular_price,p.product_description,p.stock_quantity,p.sku, group_concat(distinct pi.path) as image_paths,p.collection_id,v.id as variant_id,
       pv.product_id,group_concat(distinct (concat("'id':'",pv.id,"','product_variant_name':'",pv.product_variant_name,"','sku':'",pv.sku,"','regular_price':",pv.regular_price)) SEPARATOR ' ') 
       as combinations,v.name as variant_name, group_concat(distinct vv.value) as variant_values FROM maz_ecommerce.product
       as p inner join maz_ecommerce.product_variants as pv on p.id=pv.product_id left outer join maz_ecommerce.product_images as pi on p.id=pi.product_id 
       ...
       inner join maz_ecommerce.variant as v on vv.variant_id=v.id where p.id=? group by v.name,v.id`,
      [id],
      (error, results, fields) => {
        if (error) {
          return callBack(error, null);
  ...
  getProductWithoutVariantsById: (id, callBack) => {
    pool.query(
      `SELECT  p.name as product_name,p.supplier_id,p.category_id,p.regular_price ,p.product_description,p.stock_quantity,p.sku,p.collection_id, group_concat(distinct pi.path) as image_paths from maz_ecommerce.product as p
      left outer join maz_ecommerce.product_images as pi on p.id=pi.product_id where p.id=? group by p.id `,
      [id],
      (error, results, fields) => {
        if (error) {
          return callBack(error, null);
  ...
  getProductsByCategoryId: (id, callBack) => {
    pool.query(
      `select * from maz_ecommerce.product where category_id = ?`,
      [id],
      (error, results, fields) => {
        if (error) {
          return callBack(error, null);
  ...
  getProductsByCollectionId: (id, callBack) => {
    pool.query(
      `SELECT  p.id as id ,p.name as product_name,p.regular_price ,p.product_description,p.stock_quantity,p.sku,p.collection_id, group_concat(distinct pi.path) as image_paths from maz_ecommerce.product as p
      left outer join maz_ecommerce.product_images as pi on p.id=pi.product_id where p.collection_id=? group by p.id `,
      [id],
      (error, results, fields) => {
        if (error) {
          return callBack(error, null);
  ...
  updateProductById: (id, data) => {
    ...
    {
    pool.query(
          `update maz_ecommerce.product set name=?, category_id=?, product_description=?,supplier_id=?,sku=?,price=?,stock_status=?,quantity=?,low_stock_threshold=?,discount_price=? where id=${id}`,
          [
            data.name,
            data.category_id,
            data.product_description,
