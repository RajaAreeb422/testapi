...
addGuestUserForOrder = (data, callback) => {
  ...
  getUserByUserEmail(body.email, (err, result) => {
    ...
    if (result && result.type=='guest') {
      const salt = genSaltSync(10);
      body.password = hashSync(body.password, salt);
      body.type = "guest";
      updateGuestUser(body,result.user_id, (err, results) => {
        if (err) {
          console.log(err);
          return callback(err, null);
        }
        console.log("updated guest user", results);
       return callback(null, results.user_id);
      });
    ...
    else if(result && result.type=='user')
    {
        const m = {
        success: 0,
        message: "Please Login from your account",
      };
     return callback(m, null);
    } 
    else {
...
module.exports = {
  addOrder: (req, res) => {
    ...
    let eemail=data.userInfo.email
              ...
              sendOrderDetails(
                ...
                (err, result) => {
                  ...
                  return res.status(200).json({
                    success: 1,
                    message:
                      "order Added Successfully and  Mail Sent Successfully",
                    order: order,
                    result: result,
                  });
                }
    ...
    //  if (billing_info) {
     ...
     // } else {
       // order_info_temp["billing_id"] = shipping_info.id;
        //order_info_temp["shipping_id"] = shipping_info.id;
        promises.push(addUserAddress(data.userId, shipping_info, "billing"));
        promises.push(addUserAddress(data.userId, shipping_info, "shipping"));
        //promises.push(updateUserAddressByAddressId(shipping_info.id));
     // }
      Promise.all(promises)
      .then((address) => {
